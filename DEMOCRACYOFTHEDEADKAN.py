...
class GameApp:
    ...
    def draw_credits_roll(self):
        # クレジットロールの描画ロジック（変更なし）
        y = self.credits_y
        for height, text, color in CREDITS_CONTENT:
            pyxel.text(center_text_x(text), y, text, color)
            y += height
        
        # --- BONUS TIME! 10SEC の表示とスコア表示の統合 ---
        if self.show_final_score:
            # 1. ボーナスタイムメッセージの表示
            s_y_start = WINDOW_H // 2 - 20
            
            bonus_text = "BONUS TIME! 10SEC" 
            bonus_text_x = center_text_x(bonus_text)
            
            # テキストを点滅させる
            color = 11 if pyxel.frame_count % 8 < 4 else 8
            pyxel.text(bonus_text_x, s_y_start, bonus_text, color)

            # 2. スコア表示部分のレイアウト調整
            
            # 合計クリア時間 (ボーナスタイムタイトルの下10ピクセル + 調整)
            s_y = s_y_start + 18 
            
            t1 = "TOTAL CLEAR TIME:"
            v1 = f"{self.total_clear_time:.2f}s"
            
            pyxel.text(center_text_x(t1), s_y, t1, 7)
            pyxel.text(center_text_x(v1), s_y + 6, v1, 12)
            
            # 次回ステージ持ち越し時間
            s_y += 20 # 20ピクセル下げて次のブロックへ
            t2 = "NEXT STAGE TIME CARRY-OVER:"
            v2 = f"{self.time_remaining_next_stage:.1f}s"
            
            pyxel.text(center_text_x(t2), s_y, t2, 7)
            pyxel.text(center_text_x(v2), s_y + 6, v2, 13)
