...
# (CREDITS_CONTENT, clamp, dist, center_text_x は省略)
# (Player, Zombie, Fade, Shake クラスは省略。変更点なし)
    ...
    def update(self, obstacles, controllable=True):
        ...
        if controllable and not self.is_zombified:
            ...
            if pyxel.btn(pyxel.KEY_LEFT) or pyxel.btn(GAMEPAD_DPAD_LEFT):
                dx = -sp
                if pyxel.btn(pyxel.KEY_RIGHT) or pyxel.btn(GAMEPAD_DPAD_RIGHT):
                    dx = 0
            ...
            if pyxel.btn(pyxel.KEY_UP) or pyxel.btn(GAMEPAD_DPAD_UP):
                dy = -sp
                if pyxel.btn(pyxel.KEY_DOWN) or pyxel.btn(GAMEPAD_DPAD_DOWN):
                    dy = 0
...
class GameApp:
    def __init__(self):
        ...
        # --- SOUND DATA SETUP ---
        # BGM 0: Stage BGM - Channel 0: Melody (Pulse)
        pyxel.sounds[0].set(
            "e2e2c2g1 g1g1c2e2 d2d2d2g2 g2g2rr c2c2a1e1 e1e1a1c2 b1b1b1e2 e2e2rr",
            "p", "6", "vffn fnff vffs vfnn", 25,
        )
        # BGM 1: Stage BGM - Channel 0 (cont.): Melody 2
        pyxel.sounds[1].set(
            "r a1b1c2 b1b1c2d2 g2g2g2g2 c2c2d2e2 f2f2f2e2 f2e2d2c2 d2d2d2d2 g2g2r r ",
            "s", "6", "nnff vfff vvvv vfff svff vfff vvvv svnn", 25,
        )
        # BGM 2: Stage BGM - Channel 1: Bass (Triangle)
        pyxel.sounds[2].set(
            "c1g1c1g1 c1g1c1g1 b0g1b0g1 b0g1b0g1 a0e1a0e1 a0e1a0e1 g0d1g0d1 g0d1g0d1",
            "t", "7", "n", 25,
        )
        # BGM 3: Stage BGM - Channel 1 (cont.): Bass 2
        pyxel.sounds[3].set(
            "f0c1f0c1 g0d1g0d1 c1g1c1g1 a0e1a0e1 f0c1f0c1 f0c1f0c1 g0d1g0d1 g0d1g0d1",
            "t", "7", "n", 25,
        )
        # BGM 4: Stage BGM - Channel 2: Drums/Percussion (Noise)
        pyxel.sounds[4].set(
            "f0ra4r f0ra4r f0ra4r f0f0a4r", "n", "6622 6622 6622 6422", "f", 25
        )
        
        # BGM 5: Ending/Sanctuary Compromise BGM (Fast Pulse, Tension)
        pyxel.sounds[5].set(
            "c3c3a2f2 c3c3a2f2 g2g2e2c2 g2g2e2c2",
            "p", "7", "f", 18,
        )
        # BGM 6: Title Screen BGM (Calm/Somber)
        pyxel.sounds[6].set(
            "c2c2d2e2 e2d2c2d2 c2c2c2g1 g1g1g1r",
            "t", "6", "n", 30,
        )
        
        # SE 7: Time Up Warning (Pulse)
        pyxel.sounds[7].set(
            "c3r", "p", "7", "n", 6,
        )
        # SE 8: Zombie Captured (Triangle, High Pitch)
        pyxel.sounds[8].set(
            "c4g4", "t", "6", "s", 10,
        )
        # SE 9: Stage Clear / March Start (Square, Triumphant)
        pyxel.sounds[9].set(
            "c3e3g3c4", "s", "7", "n", 15,
        )
        # SE 10: Player Zombification / Game Over (Noise, Low)
        pyxel.sounds[10].set(
            "c0c0c0", "n", "7", "f", 12,
        )
        # SE 11: Menu Select / Enter (Pulse)
        pyxel.sounds[11].set(
            "c3", "p", "7", "n", 4,
        )
        # SE 12: Sanctuary Compromised (Noise, Strong)
        pyxel.sounds[12].set(
            "c3r", "n", "7", "s", 8,
        )
        ...
        # BGM 0: Stage BGM
        pyxel.musics[0].set([0, 1], [2, 3], [4])
        # BGM 1: Ending BGM (Sequence for tension)
        pyxel.musics[1].set([5], [], [])
        # BGM 2: Title BGM
        ...
        self.time_up_warning_played = False # 警告音制御用フラグ
        ...
        self.play_music_safe("TITLE") # 初期状態はタイトルBGM
        pyxel.run(self.update, self.draw)

    def play_music_safe(self, mode):
        """BGMの再生/停止/切り替えを行うヘルパーメソッド"""
        pyxel.stop() # 一旦全てのチャンネルを停止

        if mode == "TITLE":
            pyxel.playm(2, loop=True) # BGM 2: Title
        elif mode == "PLAYING":
            pyxel.playm(0, loop=True) # BGM 0: Stage BGM
        elif mode == "ENDING_TENSION":
            pyxel.playm(1, loop=True) # BGM 1: Ending Tension
        elif mode == "STOP":
            pass # 停止状態を維持
    ...
    def spawn_stage(self):
        ...
        if self.stage == MAX_STAGE_PLAY + 1:
            ...
            self.play_music_safe("PLAYING") # BGMスタート
            return
    ...
    def update(self):
        ...
        if self.state == "TITLE":
            ...
            if time_left < 10.0 and not self.time_up_warning_played and time_left > 0:
                pyxel.play(3, 7, loop=True) # SE 7: 警告音をループ再生
            ...
            if time_left <= 0.0 and not self.time_up_zombified:
                ...
                pyxel.stop() # BGM/警告音停止
                pyxel.play(3, 10) # SE 10: ゾンビ化/ゲームオーバー音
            ...
            if self.next_state_called and self.fade.alpha >= 0.99:
                ...
                self.state = "TITLE"
                self.fade.to(0.0, speed=0.06)
                self.play_music_safe("TITLE") # タイトルBGMへ
            ...
            if len(self.captured_zombies) == len(self.zombies) and len(self.zombies) > 0:
                ...
                self.start_march()
                self.play_music_safe("STOP") # BGM停止
                pyxel.play(3, 9) # SE 9: ステージクリア音
            ...
            if self.ending_timer == 0:
                self.fade.to(0.0, speed=0.08)
                self.play_music_safe("ENDING_TENSION") # エンディングBGM開始
            ...
            if self.ending_timer < TRANSFORM_DURATION:
                if self.ending_timer % 30 == 0:
                    pyxel.play(3, 12) # SE 12: サンクチュアリ崩壊SE (定期)
                ...
                for p in self.dummy_players:
                    if is_flashing:
                        p.temp_color = random.choice([8, 13, 3])
                    else:
            ...
            if self.ending_timer == TRANSFORM_DURATION:
                ...
                pyxel.play(3, 10) # SE 10: ゾンビ化/ゲームオーバー音 (大)
                self.play_music_safe("STOP") 
                ...
                for p in self.dummy_players:
                    ...
                    p.temp_color = None
                    for _ in range(20):
                        p.spawn_transform_particle(random.choice([8, 3, 1]))
    ...
    def draw_tutorial(self):
        ...
        begin_text = "- PRESS RETURN / GAMEPAD A/START TO BEGIN -"
        text_x = center_text_x(begin_text)
        text_y = 4
        ...
        pyxel.text(center_text_x(t2_3), y + 12, t2_3, 7)
    ...
    def draw_ending_scene(self):
        ...
        if self.ending_timer > TRANSFORM_DURATION + 30:
            ...
            t1_val_x = (BOX_X + BOX_W) - len(t1_val) * 4 - 6
            
            y = BOX_Y + (BOX_H // 2) - 4
            
            pyxel.text(t1_x, y, t1, 7)
            pyxel.text(t1_val_x, y, t1_val, 7)

    def draw_credits_roll(self):
        y = self.credits_y
        for height, text, color in CREDITS_CONTENT:
            pyxel.text(center_text_x(text), y, text, color)
            y += height
        
        if self.show_final_score:
            s_y_start = WINDOW_H // 2 - 20
            s_text = "BONUS TIME! 10SEC ADDED"
            pyxel.text(center_text_x(s_text), s_y_start, s_text, 13)
            pyxel.text(center_text_x(s_text), s_y_start + 8, "WELL DONE...", 7)

            final_time = f"TOTAL CLEAR TIME: {self.total_clear_time:.2f}s"
            pyxel.text(center_text_x(final_time), s_y_start + 30, final_time, 8)
