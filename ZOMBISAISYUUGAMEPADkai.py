...
class Player:
    ...
    def update(self, obstacles, controllable=True):
        ...
        if controllable and not self.is_zombified:
            ...
            # --- 🎮 プレイヤー移動入力 (修正適用箇所: 参考コードのスタイルを適用) ---
            
            # X軸の入力 (左右のボタンが両方押されたら、どちらか一方に優位性を持たせる)
            ...
            # XとYが両方入力されている場合、斜め移動の速度を補正する
            if dx != 0 and dy != 0:
                # 速度を対角線移動時に約70.7%に抑える (対角線補正)
...
class GameApp:
    ...
    def draw_credits_roll(self):
        ...
        if self.show_final_score:
            ...
            pyxel.text(center_text_x(time_text), y_start, time_text, 10)
            pyxel.text(center_text_x(remaining_text), y_start + 8, remaining_text, 7)
            pyxel.text(center_text_x(next_time_text), y_start + 16, next_time_text, 7)
