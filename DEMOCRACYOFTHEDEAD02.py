...
class GameApp:
    ...
    def draw_credits_roll(self):
        ...
        if self.show_final_score:
            ...
            # 3行目: 次のステージ制限時間 (+ボーナス)
            next_time_text = f"NEXT STAGE TIME: {self.time_remaining_next_stage:.1f}s (+{BONUS_TIME_AFTER_CLEAR:.1f}s BONUS)" 
            
            y_start = WINDOW_H // 2 - 20
            
            # 中央に表示
            pyxel.text(center_text_x(time_text), y_start, time_text, 7)
            pyxel.text(center_text_x(remaining_text), y_start + 14, remaining_text, 10) # 14ピクセル下にずらす
            pyxel.text(center_text_x(next_time_text), y_start + 28, next_time_text, 11) # さらに14ピクセル下にずらす
